---This class allows easier creation of configuration options in the game settings page.
...
---v1.5: fix a crash which happened when another mod called g_gui:setGui(nil) -- by Timmeey86 (https://github.com/Timmeey86/FS25_UnloadBalesEarly/commit/92b0fe24dfa3728b59d8b2796d7b19d190d44f7f)
---@class UIHelper
UIHelper = {}

---Creates a new section with the given title
---@param gameSettingsPage table @The game settings page of the base game
---@param i18nTitleId string @The I18N ID of the title to be displayed
---@return table|nil @The created section element
function UIHelper.createSection(gameSettingsPage, i18nTitleId)
	local sectionTitle = nil
	for _, elem in ipairs(gameSettingsPage.gameSettingsLayout.elements) do
		if elem.name == "sectionHeader" then
			sectionTitle = elem:clone(gameSettingsPage.gameSettingsLayout)
	...
	if sectionTitle then
		...
		table.insert(gameSettingsPage.controlsList, sectionTitle)
	end
	return sectionTitle
end
...
local function createElement(gameSettingsPage, template, id, i18nTextId, target, callbackFunc)
	local elementBox = template:clone(gameSettingsPage.gameSettingsLayout)
	...
	-- => Since we want to allow any target for callbacks, we just copy the game settings page's name to the target
	target.name = gameSettingsPage.name
	...
	table.insert(gameSettingsPage.controlsList, elementBox)
	return elementBox
end


---Adds a simple yes/no switch to the UI
---@param gameSettingsPage   table       @The base game object for the settings page
...
function UIHelper.createBoolElement(gameSettingsPage, id, i18nTextId, target, callbackFunc)
	return createElement(gameSettingsPage, gameSettingsPage.checkWoodHarvesterAutoCutBox, id, i18nTextId, target, callbackFunc)
end
...
---@return                      table       @The created object
function UIHelper.createChoiceElement(gameSettingsPage, id, i18nTextId, i18nValueMap, target, callbackFunc, nillable, prefix)
	local choiceElementBox = createElement(gameSettingsPage, gameSettingsPage.multiVolumeVoiceBox, id, i18nTextId, target, callbackFunc)
...
---@param nillable              string      @If set to true, the first entry will mean the setting has no effect. The text value will be "-".
---@return                      table       @The created object
function UIHelper.createRangeElement(gameSettingsPage, id, i18nTextId, minValue, maxValue, step, unit, target, callbackFunc, nillable)
	local rangeElementBox = createElement(gameSettingsPage, gameSettingsPage.multiVolumeVoiceBox, id, i18nTextId, target, callbackFunc)
...
---@param settingsPage table @The game settings page
...
function UIHelper.getRangeValue(control, controlState)
	if control.nillable and controlState == 1 then
		...
		--local res = string.format("%.1f", control.min + control.step * (controlState - offset))
		--return res
